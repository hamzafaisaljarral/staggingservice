    ...
    def get_upbit_ohlc(self, period, product_code, market, seconds, time_from):
        ...
        for step in range(1, 11):
            ...
            if to >= datetime.datetime.now():
                ...
                count=abs(count)
                print("the count",count)  # period의 second로 나누어서 가져올 ohlc의 갯수를 지정
                _break = True  # 현재시간까지 다 왔으니 break
            if period in ('1M', '5M', '15M', '30M', '1H', '4H'):
                url = coin.do_request("GET","/kline",{
                    "symbol":"plf_usdt",
                    "period":"1"
            ...
            if url != None:
                log.w('gray', data)
                for upbit_ohlc in data['data']:
                    #print("inside for")
                    _time = int(upbit_ohlc[0])
                    print(_time)
                    ohlc_dict[_time] = {
                        'time': _time,
                        'open': upbit_ohlc[2], #- (upbit_ohlc[2] % mpu)
                        'high': upbit_ohlc[3], #- (upbit_ohlc[3] % mpu),
                        'low': upbit_ohlc[4], #- (upbit_ohlc[4] % mpu),
                        'close': upbit_ohlc[5], #- (upbit_ohlc[5] % mpu),
                        'volume': upbit_ohlc[1], #- (upbit_ohlc[1] % msu),
                    }
                    #print("upbitohlc",ohlc_dict)    
            else:
